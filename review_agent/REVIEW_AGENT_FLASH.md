...
```
uv run python rt.py claim <代理ID> [场景 函数]   # 领任务包：认领待办块+返回精简工作包(租约防撞; 无场景参数=自动取队列首块)
uv run python rt.py pack <场景> <函数>           # 丢包时重取工作包(不占租约)
uv run python rt.py runcheck <场景> <函数>       # 自动体检：序号断裂/倒序/跨场景/**take缺口候选**/文本低相似/复用分歧
uv run python rt.py vid <10位语音ID>            # take档案：结构定位/官方复用structure_refs/补录文本/AT9存在性(matched_by_pipeline是循环证据勿采信)
uv run python rt.py find <文本> --char 003 --scene 047   # 检索EVO台词(--char=角色码=vid前3位; --scene=语音场景3位或EVO场景名均可; --evoscene同义)
...
5. 检索范围 = evo_structure + additional（未引用录音登记表：EVO 脚本未引用、但音频文件真实存在的录音，文本为转写、**可能有误听**）。find/findmany 命中 `scene:"additional"`、`vid` 返回 `source:"additional"` 均为这类录音。B 类行对补充表的唯一 sim=100 命中 + 锚点/语境佐证 → 同样可 FOUND+correct_vid；转写文本相似度略低于全等（如误听/标点差）时可结合语境裁量。`vid` 查无时看提示：`at9_exists:true`+`unreferenced_take` = 录音存在但无文本（听辨级，报 SUSPECT/UNRESOLVED 附注）；连 AT9 都没有才算真不存在。
6. EV 事件块常有"平行分支会话"：对齐器易把**相邻块同文本 take** 错配到本块，而本块自己的录音组 take 成为孤儿。信号：现配 vid 的 evo_block/录音组与本块锚点 run 不符、`vid` 查得 matched_by_pipeline 为空且与 run 内正句成对 → `WRONG` + correct_vid（填孤儿 take）。
...
1. 领块：有分配清单时 `claim <代理ID> <场景> <函数>` 直接取工作包；自主领块用 `claim <代理ID>`（自动取队列首块）。领到后 `runcheck` 体检——issue 行就是优先核查对象。。
   - **`take缺口候选` 裁定规则**：runcheck 给出 `gap_vid/gap_text`（邻行序号断口处存在的录音）。改配前必须核对 `gap_text` 与本行台词：**文本全等或高度相似（≥85）才可 `WRONG`+correct_vid=gap_vid**；gap_text 是别的台词（拟声词不同/完全异文）则缺口支不属于本行——维持现配 `OK` 并在 reason 注明"缺口take文本不符"。`rowhint` 的 `group_conflict:true`（现配组与邻锚组不一致）是同一信号，配套使用。
2. **已匹配行**（有 vid）：逐条核对文本（pack 行内 text vs evo_text）+ runcheck 的 issues：
   ...
   - `跨场景` issue：用 `vid <ID>` 查官方复用——判据是 **`structure_refs` 多处出现/`official_reuse:true`**（该 vid 被 EVO 结构多处场景内嵌）；`matched_by_pipeline` 是我方匹配结果回显（循环证据），**不能**当复用依据 → structure_refs≥2 则 `OK`
   - `文本低相似`：用 `vid` 看该 take 的原生文本（`text` 字段）比对；确属不同台词 → `WRONG`/`SUSPECT`
...
## submitmany 格式（JSON数组，注意引号转义；单条裁定也用它）

```
uv run python rt.py submitmany '[{"RemakeVoiceID":"102345","task":"A","verdict":"OK","reason":"runcheck通过,文本全等"}]'
uv run python rt.py submitmany '[{"RemakeVoiceID":"102346","task":"A","verdict":"WRONG","correct_vid":"0010470181","reason":"锚点run 0180-0182,现配0143在run外"},{"RemakeVoiceID":"102350","task":"B","verdict":"CANDIDATES","candidates":[{"vid":"0030281695","evidence":"跨场景同文"}],"reason":"两支同文无法区分"}]'
```

verdict 取值：`OK / WRONG(须给correct_vid) / SUSPECT / FOUND(须给correct_vid) / CANDIDATES / NO_VOICE / UNRESOLVED`

补充：`NO_VOICE` 仅在确证 EVO 结构存在对应行但未配音时使用（find 不索引无 voice_id 的行，需由 pack 邻域或 vid 查证发现）；查证不到就 `UNRESOLVED`，不硬判。注意：带 `--char/--scene/--evoscene` 过滤的 find/findmany **不会返回未配音结构行**——要确证 NO_VOICE 需用**无过滤**检索（或 vid/rowhint）；大量 NPC 对话块因此整块 NO_VOICE 属正常结论，快速提交即可。

## 节奏

- 一次只做一个块，做完整块 submitmany 一次提交，然后 `claim <代理ID>` 领下一块。
