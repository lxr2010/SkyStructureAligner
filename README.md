...
pure_evo 的价值**不在匹配层**（唯一确定数与 main 相差 <0.2%），而在**校对层**——它让自动校对智能体的每一次判断都有原生证据可依：

| 优势 | main 分支 | pure_evo 分支 | 效果 |
|------|------|------|------|
| **vid 归属权威** | vid→行映射来自 SoraVoice 转手数据，无法自证 | vid→行映射来自 EVO 原生 MBIN V 标记 | `vid` 命令返回 `structure_refs`（该 vid 在 EVO 结构中被哪些场景引用）——官方复用不再是猜测 |
| **录音存在性** | "查不到" = 无法区分"不存在"和"没找到" | AT9 清单 44,839 支全量索引 | `vid` 返回 `at9_exists` + `unreferenced_take`（录音存在但未被脚本引用）——W5A1 类 NO_VOICE 误判被结构性消除 |
| **take 缺口候选** | 校对员靠人眼观察"序号跳了一位" | `runcheck` 自动检测断口序号，若对应录音存在则给出 `gap_vid/gap_text` | 候选直接附在体检报告里，校对员只需比对文本相似度 |
| **组冲突警报** | 无 | `rowhint` 返回 `group_conflict: true`（现配组与邻锚组不符 = 跨场景支嫌疑） | 老需要三步查证的疑点现在一行 JSON 直出 |
| **说话人身份** | bank 名字靠文本鉴别（人工 7 个） | bank 名字靠原生 speaker 投票 + cast 表反查 + 名框投票三层命名（25 个） | `bank` 命令直接出名字（含中文名），无需查外部网站 |
| **人工中文名** | 无 | `MANUAL_CN` 表维护 8 个 bank 的日中对照 | 审查表可读性提升 |

**校对效果实测**（二轮校对后，B/C/D 桶仲裁以 pure 原生文本为裁判）：
- B 桶 604 行：**我方文本全等 400 : 基准文本全等 18**（22:1）
- C 桶 368 行：**81% 我方文本全等**（净增益，来自 pure 修复漏标 + FOUND 补配）
- D 桶 181 行：**基准文本全等仅 3%**（176 行基准所配 take 与本行台词文本不一致）
- **有效内容正确：我方 17,666 vs 基准 16,991（+675）**

详细对比分析报告（含 B/C/D 桶逐行示例）见 [docs/baseline_comparison_report.md](docs/baseline_comparison_report.md)。
